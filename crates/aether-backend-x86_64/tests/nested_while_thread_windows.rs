...
#[test]
fn windows_nested_while_in_function_emits_unique_labels() {
    ...
    assert!(asm.contains("LWH_HEAD_worker_0:"), "missing outer while head");
    assert!(asm.contains("LWH_END_worker_0:"), "missing outer while end");
    assert!(asm.contains("LWH_HEAD_worker_1:"), "missing inner while head");
    assert!(asm.contains("LWH_END_worker_1:"), "missing inner while end");
}
